...
class QSSHighlighter(QSyntaxHighlighter):
    ...
    RULES = [
        (r'\b(QWidget|QMainWindow|QDialog|QDockWidget|QToolBar|QMenuBar|QMenu'
         ...
         r'|QgsMapCanvas|QgsLayerTreeView)\b', _fmt("#4fc1e9", bold=True)),
        (r'::(hover|pressed|checked|unchecked|selected|disabled|focus'
         ...
         r'|indicator|separator|branch|add-line|sub-line)\b', _fmt("#a29bfe")),
    ...
    def __init__(self, document):
        ...
        self._hex_rx = QRegularExpression(r'#([0-9a-fA-F]{8}|[0-9a-fA-F]{6}|[0-9a-fA-F]{3})\b')
        self._rgb_rx = QRegularExpression(r'rgba?\(\s*\d+\s*,\s*\d+\s*,\s*\d+(?:\s*,\s*[\d.]+)?\s*\)')
    ...
    def highlightBlock(self, text):
        ...
        while it.hasNext():
            m = it.next()
            self.setFormat(m.capturedStart(), m.capturedLength(), _color_fmt(m.captured(0)))
            ...
            if len(nums) >= 3:
                try:
                    self.setFormat(m.capturedStart(), m.capturedLength(),
                                   _color_fmt(QColor(int(nums[0]),int(nums[1]),int(nums[2])).name()))
                except Exception:
                    pass
