...
from qgis.PyQt.QtGui import QColor, QFont, QIcon, QPixmap, QTextFormat
...
def _gallery_entries():
    return [
        (
            "Light",
            "Light theme",
            os.path.join(PLUGIN_DIR, "builtin_themes", "light", "light.qss"),
        ),
        (
            "Dark",
            "Dark theme",
            os.path.join(PLUGIN_DIR, "builtin_themes", "Dark", "darkstyle.qss"),
        ),
        (
            "Minimalist",
            "Minimalist theme",
            os.path.join(PLUGIN_DIR, "builtin_themes", "Minimalist", "Minimalist.qss"),
        ),
        (
            "Dark Forest",
            "Deep forest tones",
            os.path.join(PLUGIN_DIR, "builtin_themes", "DarkForest.qss"),
        ),
        (
            "Orange Forest",
            "Orange forest tones",
            os.path.join(PLUGIN_DIR, "builtin_themes", "OrangeForest.qss"),
        ),
        (
            "Wombat",
            "Wombat theme",
            os.path.join(PLUGIN_DIR, "builtin_themes", "Wombat", "stylesheet.qss"),
        ),
        (
            "Coffee",
            "Coffee theme",
            os.path.join(PLUGIN_DIR, "builtin_themes", "Coffee", "coffee.qss"),
        ),
        (
            "Dark green",
            "Dark green theme",
            os.path.join(
                PLUGIN_DIR, "builtin_themes", "Dark Green (FreeCAD)", "stylesheet.qss"
            ),
        ),
        (
            "Light green",
            "Light green theme",
            os.path.join(
                PLUGIN_DIR, "builtin_themes", "Light Green (FreeCAD)", "stylesheet.qss"
            ),
        ),
    ]
...
class QSSEditor(QPlainTextEdit):
    ...
    def _gutter_width(self):
        return 6 + self.fontMetrics().horizontalAdvance("9") * len(
    ...
    def _paint_gutter(self, event):
        ...
        while block.isValid() and top <= event.rect().bottom():
            if block.isVisible() and bot >= event.rect().top():
                ...
                p.setFont(self.font())
                p.drawText(
                    0, top, self._gutter.width() - 3, h, Qt.AlignRight, str(num + 1)
                )
            block = block.next()
            top = bot
            bot = top + int(self.blockBoundingRect(block).height())
...
class SkinKitDialog(QDialog):
    def __init__(self, iface, plugin, parent=None):
        super().__init__(parent or iface.mainWindow())
        self.iface = iface
        self.plugin = plugin
        self._hl = None
        self.setWindowTitle("SkinKit")
    ...
    def _tab_gallery(self):
        ...
        lay.addWidget(
            QLabel(
                "Select a starting theme. Click <b>Load into editor</b> then"
                " <b>Apply</b>."
            )
        )
        sc = QScrollArea()
        ...
        gl.setSpacing(6)
        for name, desc, rel in _gallery_entries():
            fr = QFrame()
            ...
            btn = QPushButton("Load into editor")
    ...
    def _load_gallery(self, name, qss_path):
        if qss_path and os.path.isfile(qss_path):
            ...
            try:
                ...
                self.iface.messageBar().pushWarning(
                    "SkinKit", f"Could not read theme file: {e}"
                )
                return
        else:
            self.qss_path_edit.clear()
            self.qss_editor.setPlainText(
                f"/* Could not locate theme file for '{name}'. */\n"
                f"/* Themes folder: {os.path.join(PLUGIN_DIR, 'builtin_themes')} */\n"
            )

        self.iface.messageBar().pushInfo(
            "SkinKit",
            f'"{name}" loaded from {qss_path or "core theme system"}. '
    ...
    def _tab_editor(self):
        ...
        lay = QVBoxLayout(w)
        lay.setContentsMargins(8, 8, 8, 8)
        grp = QGroupBox("QSS File")
        gl = QHBoxLayout(grp)
        self.qss_path_edit = QLineEdit()
        self.qss_path_edit.setPlaceholderText("External .qss file path…")
        b1 = QPushButton("Browse…")
        b1.clicked.connect(self._browse_qss)
        b2 = QPushButton("Load into editor")
        b2.clicked.connect(self._load_file_into_editor)
        for w2 in (self.qss_path_edit, b1, b2):
            gl.addWidget(w2)
        lay.addWidget(grp)
        self.live_reload_cb = QCheckBox("Live-reload — auto-apply on file save")
        lay.addWidget(self.live_reload_cb)
        eg = QGroupBox("Inline QSS editor")
        el = QVBoxLayout(eg)
        self.qss_editor = QSSEditor()
        self.qss_editor.setFont(QFont("Consolas", 10))
        self.qss_editor.setPlaceholderText(
            "/* Write or paste QSS here.\n"
            "   Hex colours are shown with background swatches.\n"
            "   Changes applied on Apply — no separate Extra QSS tab. */"
        )
        ...
        br = QHBoxLayout()
        bf = QPushButton("Auto-indent")
        bf.clicked.connect(self._format)
        bc = QPushButton("Clear")
        bc.clicked.connect(self.qss_editor.clear)
        bs = QPushButton("Save to file…")
        bs.clicked.connect(self._save_to_file)
        for b in (bf, bc, bs):
            br.addWidget(b)
        br.addStretch()
        el.addWidget(self.qss_editor)
        el.addWidget(self._val_lbl)
        el.addLayout(br)
        lay.addWidget(eg)
        return w
    ...
    def _tab_window(self):
        ...
        self.icon_path_edit.setPlaceholderText("PNG/SVG/ICO…")
        bi = QPushButton("Browse…")
        bi.clicked.connect(self._browse_icon)
        self.icon_prev = QLabel()
        self.icon_prev.setFixedSize(32, 32)
        self.icon_path_edit.textChanged.connect(self._upd_icon_prev)
        for x in (self.icon_path_edit, bi, self.icon_prev):
            ir.addWidget(x)
        form.addRow("Window icon:", ir)
        # Title
        self.title_edit = QLineEdit()
        self.title_edit.setPlaceholderText("Leave blank for default")
        form.addRow("Window title:", self.title_edit)
        # Toolbar icon size
        isr = QHBoxLayout()
        self.icon_size_sl = QSlider(Qt.Horizontal)
        self.icon_size_sl.setRange(0, 128)
        self.icon_size_sl.setSingleStep(2)
        self.icon_size_sl.setTickInterval(16)
        self.icon_size_sl.setTickPosition(QSlider.TicksBelow)
        self.icon_size_sl.setValue(0)
        self.icon_size_lbl = QLabel("default")
        self.icon_size_lbl.setFixedWidth(54)
        self.icon_size_sl.valueChanged.connect(
            lambda v: self.icon_size_lbl.setText(f"{v}px" if v > 0 else "default")
        )
        isr.addWidget(self.icon_size_sl)
        isr.addWidget(self.icon_size_lbl)
        form.addRow("Toolbar icon size:", isr)
        # Opacity
        opr = QHBoxLayout()
        self.opacity_sl = QSlider(Qt.Horizontal)
        self.opacity_sl.setRange(10, 100)
        self.opacity_sl.setValue(100)
        self.opacity_sl.setTickInterval(10)
        self.opacity_sl.setTickPosition(QSlider.TicksBelow)
        self.opacity_lbl = QLabel("100 %")
        self.opacity_lbl.setFixedWidth(42)
        self.opacity_sl.valueChanged.connect(
            lambda v: self.opacity_lbl.setText(f"{v} %")
        )
        opr.addWidget(self.opacity_sl)
        opr.addWidget(self.opacity_lbl)
        form.addRow("Window opacity:", opr)
        # Icon pack
        pkr = QHBoxLayout()
        self.icon_pack_edit = QLineEdit()
        self.icon_pack_edit.setPlaceholderText(
            "Leave blank = QGIS default icons (reference baseline)"
        )
        bp = QPushButton("Browse…")
        bp.clicked.connect(self._browse_pack)
        bd = QPushButton("Use QGIS default")
        bd.clicked.connect(lambda: self.icon_pack_edit.clear())
        for x in (self.icon_pack_edit, bp, bd):
            pkr.addWidget(x)
        form.addRow("Icon pack folder:", pkr)
        hint = QLabel(
            "Each preset stores its own icon pack path.\n"
            "Blank = QGIS built-in icons (always the reference baseline).\n"
    ...
    def _tab_background(self):
        ...
        self.bg_path_edit.setPlaceholderText("PNG/JPG/BMP path…")
        bb = QPushButton("Browse…")
        bb.clicked.connect(self._browse_bg)
        self.bg_prev = QLabel()
        self.bg_prev.setFixedSize(72, 46)
        self.bg_prev.setScaledContents(True)
        self.bg_path_edit.textChanged.connect(self._upd_bg_prev)
        for x in (self.bg_path_edit, bb, self.bg_prev):
            bgr.addWidget(x)
        form.addRow("Background image:", bgr)
        self.bg_mode = QComboBox()
        self.bg_mode.addItems(["stretch", "tile", "center", "fit"])
        form.addRow("Fill mode:", self.bg_mode)
        bor = QHBoxLayout()
        self.bg_op_sl = QSlider(Qt.Horizontal)
        self.bg_op_sl.setRange(5, 100)
        self.bg_op_sl.setValue(100)
        self.bg_op_sl.setTickInterval(10)
        self.bg_op_sl.setTickPosition(QSlider.TicksBelow)
        self.bg_op_lbl = QLabel("100 %")
        self.bg_op_lbl.setFixedWidth(42)
        self.bg_op_sl.valueChanged.connect(lambda v: self.bg_op_lbl.setText(f"{v} %"))
        bor.addWidget(self.bg_op_sl)
        bor.addWidget(self.bg_op_lbl)
        form.addRow("Image opacity:", bor)
        bc = QPushButton("Clear background")
        bc.clicked.connect(lambda: self.bg_path_edit.clear())
        form.addRow(bc)
        return w
    ...
    def _tab_font(self):
        ...
        self.font_sz = QSpinBox()
        self.font_sz.setRange(0, 72)
        self.font_sz.setSpecialValueText("(keep default)")
        form.addRow("Font size (pt):", self.font_sz)
        return w
    ...
    def _tab_presets(self):
        ...
        lay.addWidget(
            QLabel(
                "Presets store every setting"
                " (stylesheet, icon pack, background, font, opacity).\n"
            )
        )
        r1 = QHBoxLayout()
        self.preset_name = QLineEdit()
        self.preset_name.setPlaceholderText("New preset name…")
        bs = QPushButton("Save current as preset")
        bs.clicked.connect(self._save_preset)
        r1.addWidget(self.preset_name)
        r1.addWidget(bs)
        lay.addLayout(r1)
        r2 = QHBoxLayout()
        self.preset_combo = QComboBox()
        self.preset_combo.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Fixed)
        bl = QPushButton("Load into UI")
        bla = QPushButton("Load / Apply")
        bdel = QPushButton("Delete")
        bl.clicked.connect(self._load_preset_ui)
        bla.clicked.connect(self._load_apply_preset)
        bdel.clicked.connect(self._delete_preset)
        r2.addWidget(self.preset_combo)
        for b in (bl, bla, bdel):
            r2.addWidget(b)
        lay.addLayout(r2)
        lay.addStretch()
    ...
    def _load_file_into_editor(self):
        ...
        if p and os.path.isfile(p):
            try:
                ...
                self.iface.messageBar().pushWarning(
                    "SkinKit", f"Could not read file: {e}"
                )
    ...
    def _save_to_file(self):
        ...
        if p:
            try:
                ...
                self.iface.messageBar().pushWarning(
                    "SkinKit", f"Could not save file: {e}"
                )

    def _browse_icon(self):
        p, _ = QFileDialog.getOpenFileName(
            self, "Select icon", "", "Images (*.png *.svg *.ico *.jpg);;All (*)"
        )
        if p:
            self.icon_path_edit.setText(p)

    def _browse_pack(self):
        d = QFileDialog.getExistingDirectory(self, "Select icon pack folder")
        if d:
            self.icon_pack_edit.setText(d)

    def _browse_bg(self):
        p, _ = QFileDialog.getOpenFileName(
            self, "Select background", "", "Images (*.png *.jpg *.bmp *.webp);;All (*)"
        )
        if p:
    ...
    def _upd_icon_prev(self, path):
        if path and os.path.isfile(path):
            self.icon_prev.setPixmap(QIcon(path).pixmap(32, 32))
        else:
            self.icon_prev.clear()

    def _upd_bg_prev(self, path):
        if path and os.path.isfile(path):
            px = QPixmap(path)
            if not px.isNull():
                self.bg_prev.setPixmap(
                    px.scaled(72, 46, Qt.KeepAspectRatio, Qt.SmoothTransformation)
                )
                return
        self.bg_prev.clear()
    ...
    def _validate(self):
        ...
        errs = []
        if t.count("{") != t.count("}"):
            errs.append(f"Unmatched braces: {t.count('{')} vs {t.count('}')}")
        if "/*" in t and t.count("/*") != t.count("*/"):
            errs.append("Unclosed comment")
        if errs:
            self._val_lbl.setText("  ".join(errs))
            self._val_lbl.setStyleSheet("color:#e17055;")
        else:
            self._val_lbl.setText("No issues")
            self._val_lbl.setStyleSheet("color:#00b894;")
    ...
    def _save_preset(self):
        ...
        if not name:
            QMessageBox.warning(self, "SkinKit", "Enter a preset name.")
            return
        try:
            self.plugin.save_preset(name, self._collect_cfg())
        except ValueError as e:
            QMessageBox.warning(self, "SkinKit", str(e))
            return
        self._refresh_presets()
        self.preset_name.clear()
        self.iface.messageBar().pushSuccess("SkinKit", f'Preset "{name}" saved.')
    ...
    def _load_preset_ui(self):
        ...
        if name:
            self._fill_ui_from_cfg(self.plugin.load_preset(name))
    ...
    def _delete_preset(self):
        ...
        if name == "QGIS Default":
            QMessageBox.information(
                self, "SkinKit", "'QGIS Default' cannot be deleted."
            )
            return
        if (
            ...
            self.plugin.delete_preset(name)
            self._refresh_presets()
    ...
    def _on_reset(self):
        if (
            QMessageBox.question(
                self,
                "SkinKit",
                "Reset ALL SkinKit customisations?\n"
    ...
    def _safe_apply(self):
        try:
            ...
            self.iface.messageBar().pushSuccess("SkinKit", "Applied.")
        except Exception as e:
            self.iface.messageBar().pushWarning("SkinKit", f"Apply failed: {e}")
